...
inventoryController.getAllInventory = async (req, res, next) => {
  try {
    const inventory = await pool.query(
      'SELECT item_id, catalog.product_name, quantity, expiration_date FROM inventory INNER JOIN catalog ON inventory.product_id = catalog.product_id'
    );
...
inventoryController.getInventoryById = async (req, res, next) => {
  try {
    const { id } = req.params;
    const inventoryById = await pool.query(
...
inventoryController.addNewInventory = async (req, res, next) => {
  try {
    ...
    const newInventory = await pool.query(
      `INSERT INTO inventory (quantity, expiration_date, product_id) VALUES
    ($1, $2, (SELECT product_id FROM catalog WHERE product_name = $3))`,
      [quantity, expiration_date, product_name]
    );
...
inventoryController.updateInventory = async (req, res, next) => {
  try {
    ...
    const updatedInventory = await pool.query(
      'UPDATE inventory SET quantity = $1, expiration_date = $2, (SET inventory.product_name = $3 FROM catalog WHERE inventory.product_id = catalog.product_id) WHERE item_id = $4',
