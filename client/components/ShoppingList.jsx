...
function HomeContainer(props) {
  ...
  const getInventoryData = () => {
    fetch("/api/inventory")
      ...
      .catch((err) =>
        console.log("Table.componentDidMount: get tableElement: ERROR: ", err)
      );
  };
  ...
  const handleSubmit = (event) => {
    ...
    inventory.forEach((element) => {
      if (inventoryList.hasOwnProperty(element.product_name))
        inventoryList[element.product_name] += element.quantity;
    ...
    procedureData.forEach((element, index) => {
      if (element.procedure_name === procedure) {
        const total = Math.ceil(
          (numProcedures * element.qty_per_procedure) / element.qty_per_unit
        );
        let inInventory;
        if (!inventoryList.hasOwnProperty(element.product_name))
          inInventory = 0;
  ...
  procedureData.forEach((element) => {
    if (!procedureNames.includes(element.procedure_name))
      procedureNames.push(element.procedure_name);
  });
  const menuItems = [];
  ...
  if (rows.length) {
    datagrid = (
      <DataGrid
        rows={rows}
        columns={columnDefinitions.home}
        sortModel={[{ field: "needed", sort: "desc" }]}
      />
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{ height: 400, width: "100%" }}>{datagrid}</div>
    </div>
  );
}
